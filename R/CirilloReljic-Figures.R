...
ggsave(plot = fig2, 
       filename = here("output/CirilloReljic_Figure2.pdf"), 
...
ggsave(plot = fig_2, 
       filename = here("output/CirilloReljic_Figure3.pdf"), 
...
ggsave(plot = fig3, 
       filename = here("output/CirilloReljic_Figure4.pdf"), 
       width = standard_width*1.5, height = standard_heigth)
...
fig4_a <- ggplot() +
  ...
  scale_color_manual(values = c("Total EMP"="#00395B")) +
...
ggsave(plot = fig4, 
       filename = here("output/CirilloReljic_Figure5.pdf"), 
       width = standard_width*1.5, height = standard_heigth*1.2)

# Figure 6-----------------------------
# Evolution of non-standard forms of work 
# Source: Authors’ elaboration based on the EU LFS
fig5_data <- read_xlsx(here("data/ReljicCirillo/Figure 5.xlsx")) %>% 
  dplyr::mutate(
    region_group = ordered(
      region_group, levels=c("Total", "South","Central","Northwest", "Northeast"))
  )

fig5 <- fig5_data %>% 
  ggplot(
    data = ., aes(x=year, y=qnsw, color=region_group)
  ) +
  geom_line(key_glyph = draw_key_rect) + geom_point(size=0.75, alpha=0.5) +
  scale_color_euf(palette = "mixed") +
  scale_x_continuous(breaks = seq(2000, 2022, 2)) +
  scale_y_continuous(
    labels = scales::label_percent(scale = 1), limits = c(20, 38)) +
  labs(
    title = "Evolution of non-standard forms of work", 
    y="Share of non-standard workers", 
    caption = "Source: EU LFS; authors' own computation." ) +
  theme_jahrbuch 
fig5

ggsave(plot = fig5, 
       filename = here("output/CirilloReljic_Figure6.pdf"), 
...
fig6 <- fig6_data %>% 
  ggplot(
    data = ., aes(x=year, y=value, color=variable)
  ) +
  geom_line(key_glyph = draw_key_rect) + geom_point(size=0.75, alpha=0.5) +
  scale_color_euf(palette = "mixed") +
...
ggsave(plot = fig6, 
       filename = here("output/CirilloReljic_Figure7.pdf"), 
       width = standard_width*1.2, height = standard_heigth)
...
fig7a <- fig7_data %>% 
  ggplot(data = ., aes(x=LRUR_LF, y=AROP, color=group)) +
  geom_point(size=0.8, key_glyph=draw_key_rect) +
...
fig7b <- fig7_data %>% 
  ggplot(data = ., aes(x=AR, y=AROP, color=group)) +
  geom_point(size=0.8, key_glyph=draw_key_rect) +
  ggrepel::geom_text_repel(
    mapping = aes(label=region_lab), seed = 123, size=2.2, 
    max.overlaps = 15, min.segment.length = 0.2, key_glyph=draw_key_rect
...
fig7c <- fig7_data %>% 
  ggplot(data = ., aes(x=TEMP_PT, y=AROP, color=group)) +
  geom_point(size=0.8, key_glyph=draw_key_rect) +
  ggrepel::geom_text_repel(
    mapping = aes(label=region_lab), seed = 123, size=2.2, 
    max.overlaps = 15, min.segment.length = 0.2, key_glyph=draw_key_rect
...
fig7d <- fig7_data %>% 
  ggplot(data = ., aes(x=QLOW_INT, y=AROP, color=group)) +
  geom_point(size=0.8, key_glyph=draw_key_rect) +
  ggrepel::geom_text_repel(
    mapping = aes(label=region_lab), seed = 123, size=2.2, 
    max.overlaps = 15, min.segment.length = 0.2, key_glyph=draw_key_rect
...
ggsave(plot = fig7, 
       filename = here("output/CirilloReljic_Figure8.pdf"), 

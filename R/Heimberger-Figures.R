here::i_am("R/Heimberger-Figures.R")
library(here)
library(data.table)
library(countrycode)
library(magrittr)
...
fig1_data <- fread(here("data/Heimberger/Figure1_bookchapter.csv")) %>% 
  as_tibble() %>% 
  pivot_longer(cols = -"year", names_to = "country", values_to = "LaborProductivity") %>% 
  dplyr::filter(!is.na(LaborProductivity))

fig1 <- fig1_data %>% 
  ggplot(data = ., mapping=aes(x=year, y=LaborProductivity, color=country)) +
  geom_line(key_glyph=draw_key_rect) + geom_point(size=0.75) +
...
ggsave(
  ...
  filename = here("output/Heimberger_Figure1.pdf"))
...
fig2 <- fig2_data %>% 
  ggplot(data = ., mapping=aes(x=year, y=Ratio, color=Relation)) +
...
ggsave(
  ...
  width = standard_width, 
  height = standard_heigth, 
  filename = here("output/Heimberger_Figure2.pdf"))
...
fig3 <- fig3_data %>% 
  ggplot(data = ., mapping=aes(x=year, y=Demand, color=Country)) +
...
ggsave(
  ...
  width = standard_width, 
  height = standard_heigth, 
  filename = here("output/Heimberger_Figure3.pdf"))
...
fig4 <- fig4_data %>% 
  ...
  geom_vline(xintercept = 2020, color=get_euf_colors("grey"), linetype="longdash") +
  geom_line(
    key_glyph=draw_key_rect, mapping = aes(
      x=V1, y=lrate, 
  ...
  geom_point(size=0.75, mapping = aes(
    x=V1, y=lrate, 
    color="10-year-government bond yield")) +
  geom_point(
    size=0.75, mapping = aes(
      x=V1, y=pdebt/10, 
      color="Public debt to GDP")) +
  scale_color_manual(values = c(
    "10-year-government bond yield"=get_euf_colors(palette_used = "mixed", n = 2)[1],
    "Public debt to GDP"=get_euf_colors(palette_used = "mixed", n = 2)[2])) +
  scale_x_continuous(
    breaks = seq(1960, 2020, 10)
  ) +
  scale_y_continuous(
...
ggsave(
  ...
  width = standard_width, 
  height = standard_heigth, 
  filename = here("output/Heimberger_Figure4.pdf"))
...
fig5 <- fig5_data %>% 
  ...
  geom_vline(xintercept = 2010, color=get_euf_colors("grey"), linetype="longdash") +
  geom_vline(xintercept = 2020, color=get_euf_colors("grey"), linetype="longdash") +
  geom_line(key_glyph=draw_key_rect, color=get_euf_colors("blue")) + 
  geom_point(size=0.75, color=get_euf_colors("blue")) +
  scale_color_euf(palette = "mixed") +
  scale_x_continuous(
    breaks = seq(1960, 2020, 10)
  ) +
...
ggsave(
  ...
  width = standard_width, 
  height = standard_heigth, 
  filename = here("output/Heimberger_Figure5.pdf"))
...
fig6 <- fig6_data %>% 
  ...
  scale_color_euf(palette = "mixed", aesthetics = c("color", "fill")) +
  scale_y_continuous(labels = label_percent(scale = 1)) +
  labs(
...
ggsave(
  ...
  width = standard_width, 
  height = standard_heigth, 
  filename = here("output/Heimberger_Figure6.pdf"))
...
fig7a <- fig7a_data %>% 
  ggplot(data = ., mapping=aes(x=year, y=`EPL index`, color=country)) +
  #geom_hline(yintercept = 100, color=get_euf_colors("grey")) +
  geom_vline(xintercept = 2007, color=get_euf_colors("grey"), linetype="longdash") +
  geom_vline(xintercept = 2010, color=get_euf_colors("grey"), linetype="longdash") +
  geom_line(key_glyph=draw_key_rect) + geom_point(size=0.75) +
...
ggsave(
  ...
  filename = here("output/Heimberger_Figure7.pdf"))
...
ggsave(
  ...
  width = standard_width, 
  height = standard_heigth, 
  filename = here("output/Heimberger_Figure8.pdf"))
...
fig9 <- fig9_data %>% 
  ggplot(data = ., mapping=aes(x=year, y=RealComp, color=country)) +
  geom_vline(xintercept = 2007, color=get_euf_colors("grey"), linetype="longdash") +
  geom_vline(xintercept = 2010, color=get_euf_colors("grey"), linetype="longdash") +
  geom_vline(xintercept = 2020, color=get_euf_colors("grey"), linetype="longdash") +
  geom_line(key_glyph=draw_key_rect) + geom_point(size=0.75) +
...
ggsave(
  ...
  width = standard_width, 
  height = standard_heigth, 
  filename = here("output/Heimberger_Figure9.pdf"))
